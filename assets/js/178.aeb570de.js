...
__webpack_require__(6121)((lunr_default()));__webpack_require__(7923)((lunr_default()));__webpack_require__(6106)((lunr_default()));const removeDefaultStopWordFilter=false;const language=["en","fr"];const searchIndexUrl="search-index{dir}.json?_=50463aff";const searchResultLimits=8;const fuzzyMatchingDistance=1;
...
const extraTerms=[];for(const term of refinedTerms){if(term.length>2){for(let i=term.length-1;i>=0;i-=1){extraTerms.push(term.slice(0,i).concat(term.slice(i+1)));}}}return getQueriesMaybeTyping(terms).concat(fuzzyMatchingDistance>0?getQueriesMaybeTyping(terms,fuzzyMatchingDistance):[],getQueriesMaybeTyping(extraTerms),fuzzyMatchingDistance>0?getQueriesMaybeTyping(extraTerms,fuzzyMatchingDistance):[]);}function getQueriesMaybeTyping(terms,editDistance){return termsToQueries(terms,false,editDistance).concat(termsToQueries(// Ignore terms whose last token already has a trailing wildcard,
// or the last token is not `maybeTyping`.
terms.filter(term=>{const token=term[term.length-1];return!token.trailing&&token.maybeTyping;}),true,editDistance));}function termsToQueries(terms,maybeTyping,editDistance){return terms.flatMap(term=>{const query={tokens:term.map(item=>item.value),term:term.map(item=>({value:item.value,presence:(lunr_default()).Query.presence.REQUIRED,// The last token of a term maybe incomplete while user is typing.
// So append more queries with trailing wildcard added.
wildcard:(maybeTyping?item.trailing||item.maybeTyping:item.trailing)?(lunr_default()).Query.wildcard.TRAILING:(lunr_default()).Query.wildcard.NONE,editDistance:editDistance&&item.value.length>editDistance?editDistance:undefined}))};// Ignore queries that all terms ignored edit distance due to too short tokens.
if(editDistance&&query.term.every(item=>!item.editDistance)){return[];}return query;});}
