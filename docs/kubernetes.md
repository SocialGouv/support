...
Pour la _production_ pensez à bien à vérifier le `namespace` et le `secret name` spécifié. Le `secret name` est le nom du secret lié à l'application, par exemple `app`, `api` ou  `app-sealed-secret` ; on peut trouver ce nom dans la configuration des workflows (par exemple dans le fichier `.kube-workflow/values.yaml`, dans la partie `secretRef` sous `envFrom`).
